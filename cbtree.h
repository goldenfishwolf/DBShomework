...
#ifndef DATABASE_MID_CBTREE_H
#define DATABASE_MID_CBTREE_H
#pragma once

class CBTree
{
private:
    attribute**  tableArray[MAX_ITEM];
    int itemCount;
    int attrCount;
public:
    CBTree(){
        ...
        for(int i = 0; i < MAX_ITEM; i++) {
            tableArray[i] = new attribute *[MAX_ATTRIBUTE];
            for (int j = 0; j < MAX_ATTRIBUTE; j++)
                tableArray[i][j] = new attribute;
        }
    }
    bool insert(attribute** newItem){
        //cout <<"table insert"<<endl;
        int count = 0;

        while(newItem[count]->type!="end"){
            //cout<<"check key"<<endl;
            if(newItem[count]->isKey) {
                //cout << "check whether the key has already used"<<endl;
               // cout <<"-----------input key:" << newItem[count]->content<<endl;
                for (int i = 0; i < itemCount; i++) {
                    if (tableArray[i][count]->content == newItem[count]->content) {
                        cout << "existed same key, insert failed" << endl;
                        return false;
                    }
                }
            }
        ...
        if(itemCount < 10){
            tableArray[itemCount] = newItem;
            itemCount++;
            //cout << "prepare to insert into table" <<endl;
        }else{
            cout<<"too many item"<<endl;
            return false;
        }
        return true;
    }
    void display(){
        cout<< "attrCount: " <<attrCount<< " itemCount: " << itemCount<<endl;
        cout <<"----------------------------------"<<endl;
        for(int i = 0; i < itemCount;i++){
            for(int j = 0; j < attrCount;j++){
                cout << "   "<<  tableArray[i][j]->field<<"   "<<tableArray[i][j]->content;
            }
            cout <<endl;
        }
        cout <<"----------------------------------"<<endl;
...
public:
    ...
    void serAttrNum(int a) {attributeNum = a;}
    bool insert(const T &key)    //向B数中插入新结点key
    {
        cout <<"start insert in the tree"<<endl;
